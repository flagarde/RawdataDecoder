...
void ROOTWriter::processCell(const Data& d, const std::uint32_t& chip, const std::uint32_t& channel)
{
  ...
  for(std::size_t i = 0; i != d.getChip(chip).getNumberColumns(); ++i)
  {
    ...
    if(d.getLayer() != myCellID.getLayerID() || d.getChip(chip).getID() != myCellID.getChipID() || i != myCellID.getMemory() || channel != myCellID.getChannel()) throw("Problem encoding CellID");
